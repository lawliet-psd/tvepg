...
def fetch_epg_ids():
    ...
    for ch in root.findall("channel"):
        ...
        if disp and cid:
            key = clean_name(disp)
            epg_ids[key] = {'id': cid, 'display': disp}
    return epg_ids
    ...
    for epg_key, aliases in ALTERNATIVE_NAMES.items():
        for alias in aliases:
            if clean_name(alias) == cleaned:
                return epg_map.get(epg_key, None)
...
def process_m3u(m3u_data, epg_map):
    ...
    for i in range(len(lines)):
        ...
        if line.startswith("#EXTINF:"):
            ...
            epg_info = fuzzy_match(ch_name, epg_map)
            ...
            if epg_info:
                tvg_id = epg_info['id']
                display_name = epg_info['display'].upper()
                line = re.sub(r'#EXTINF:-1', f'#EXTINF:-1 tvg-id="{tvg_id}"')
                line = re.sub(r',(.*)$', f',{display_name}')
            else:
